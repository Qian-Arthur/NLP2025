...
# 2. 加载并预处理数据
folder_path = r"C:\Users\WXQ\Desktop\2025春\jyxstxtqj_downcc.com"  # 替换为存放金庸小说的文件夹路径
...
y = np.array([seq[1:] for seq in sequences])  # 修改标签为每个字符的标签
...
def transformer_model(sequence_length, total_chars, num_heads=8, ff_dim=512):
    ...
    # 输出层
    output = Dense(total_chars, activation='softmax')(output)  # 修改输出层为每个时间步的类别概率分布
...
# 8. 训练Transformer模型
model_transformer.fit(X, y, batch_size=64, epochs=5)
