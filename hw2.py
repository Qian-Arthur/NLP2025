...
# ================= 参数配置 =================
CORPUS_DIR = r"C:\Users\WXQ\Desktop\jyxstxtqj_downcc.com"          # 小说存放目录
...
def sample_chunks(novels, k, unit='word'):
    ...
    for novel_id, (fname, text) in enumerate(novels.items()):
        ...
        if unit == 'word':
            ...
            #tokens = list(jieba.cut(text))  # 分词处理
        else:
            tokens = [w for w in list(text) if w not in stopwords] # 按字符处理
...
def run_experiment():
    ...
    # 替换分类器为随机森林，推荐设置关键参数
                    clf = RandomForestClassifier(
                                            n_estimators=200,       # 增加树的数量提升性能
                                            max_depth=15,           # 控制树复杂度防止过拟合
